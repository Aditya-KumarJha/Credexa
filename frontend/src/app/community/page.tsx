...
function EventRow({ date, name, type }) {
	...
	return (
		<tr className="border-b">
			...
			<td className="p-2">
				<button
					className={`px-3 py-1 rounded ${rsvp ? "bg-green-500 text-white" : "bg-blue-100 text-blue-700 hover:bg-blue-200"}`}
			...
			<td className="p-2">
				<button
					className={`px-3 py-1 rounded ${reminder ? "bg-purple-500 text-white" : "bg-blue-100 text-blue-700 hover:bg-blue-200"}`}
...
export default function CommunityPage() {
	const [section, setSection] = React.useState("discussion");
	// Discussion Section
	...
	// Peer Groups Section
		...
		const [joinedGroup, setJoinedGroup] = React.useState(null);
				...
				<nav className="mb-6 flex justify-center gap-2 border-b pb-2">
					{[
						...
						<button
							...
							className={`px-4 py-1 rounded font-medium transition-all duration-150 ${section === tab.key ? "bg-blue-600 text-white" : "bg-blue-50 text-blue-700 hover:bg-blue-100"}`}
						>
							{tab.label}
						</button>
					))}
									...
									<div key={post.id} className="mb-4 bg-white rounded-lg p-3 border border-gray-200 shadow flex gap-3">
										...
										<div className="flex-1">
											...
											<div className="flex gap-2 mt-2 items-center">
												<button
													className={`w-6 h-6 flex items-center justify-center rounded transition ${post.liked ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
													onClick={() => handleLike(post.id)}
													title="Like"
													style={{border: 'none', outline: 'none', position: 'relative'}}
												>
													<svg width="14" height="14" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
														<path d="M6.5 14V7.5L10.5 2.5C10.5 2.5 11.5 2.5 11.5 4.5C11.5 6.5 9.5 7.5 9.5 7.5H13C13.5 7.5 14 8 14 8.5V13C14 13.5 13.5 14 13 14H6.5Z" stroke={post.liked ? '#2563eb' : '#222'} strokeWidth="1.2" fill={post.liked ? '#2563eb' : 'white'} />
													</svg>
												</button>
