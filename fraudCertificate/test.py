...
# Set device to CPU (change to 'cuda' if CUDA is available)
...
class Model(nn.Module):
    ...
    def load(self, path=''):
        try:
            ...
            print(f"Warning: Could not find pretrained weights at {self.save_dir + path + '%s_weights.pth' % self.networks.name}")
            print("Please download the pretrained weights from:")
            print("Google Drive: https://drive.google.com/file/d/1scOAVxvqYSfRi4s7s0crk2Ieqn5Cm6r6/view?usp=sharing")
            print("Baidu Pan (Code: n2ai): https://pan.baidu.com/s/17W14E3pQLyPia14Kq8QeQg")
